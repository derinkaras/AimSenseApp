...
export default function Step7() {
  ...
  if (phase === "verify") {
    ...
    // Get the end position that was just calibrated (elevation or windage depending on axes)
    const dialedEndPx = calibratingElevation
        ? useCalibrationStore.getState().elevationEndPx
        : useCalibrationStore.getState().windageEndPx;
                  ...
                  {verifyPos && (
                      <>
                        ...
                        <View style={{ position: "absolute", left: verifyPos.x - 40, top: verifyPos.y + 45, backgroundColor: "rgba(0,0,0,0.75)", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: "rgba(11,127,79,0.5)" }} pointerEvents="none">
                          <Text style={{ color: "#22c55e", fontSize: 11, fontFamily: "monospace", fontWeight: "600" }}>ZERO ({verifyPos.x}, {verifyPos.y})</Text>
                        </View>
                      </>
                  )}
                  {/* Dialed position - ORANGE (only show if different from zero) */}
                  {dialedEndPx && verifyPos && (dialedEndPx.x !== verifyPos.x || dialedEndPx.y !== verifyPos.y) && (
                      <>
                        <View
                            style={[
                              crosshairStyles.container,
                              { left: dialedEndPx.x - 60, top: dialedEndPx.y - 60 }
                            ]}
                            pointerEvents="none"
                        >
                        ...
                        <View style={{ position: "absolute", left: dialedEndPx.x - 40, top: dialedEndPx.y + 45, backgroundColor: "rgba(0,0,0,0.75)", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: "rgba(249,115,22,0.5)" }} pointerEvents="none">
                          <Text style={{ color: "#f97316", fontSize: 11, fontFamily: "monospace", fontWeight: "600" }}>DIALED ({dialedEndPx.x}, {dialedEndPx.y})</Text>
                        </View>
                      </>
                  )}
                </View>
          ...
          {isLandscapeMode ? (
              <SafeAreaView className="absolute top-0 bottom-0 right-0 bg-brand-black/95 border-l border-brand-green/30" edges={["top", "bottom", "right"]} style={{ width: layoutConfig.sidePanelWidth }}>
                <View className="flex-1 px-3 pt-3">
                  <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                    ...
                    <SectionCard variant="secondary" compact className="mt-2"><Text className="text-white/80 text-[10px] leading-3.5 text-center">The green marker shows your original zero. The orange marker shows where you dialed. After dialing back, your scope should align with green.</Text></SectionCard>
                    {verifyPos && <SectionCard variant="elevated" compact className="mt-2"><Text className="text-white/50 text-[10px] text-center">Original Zero</Text><Text className="text-brand-greenLight font-mono text-lg font-bold text-center mt-1">({verifyPos.x}, {verifyPos.y})</Text></SectionCard>}
                    <View className="bg-brand-greenLight/10 border border-brand-greenLight/30 rounded-xl p-2 mt-2"><Text className="text-brand-greenLight/90 text-[10px] text-center font-medium">If aligned with GREEN, tap Continue. If not, go back and re-dial.</Text></View>
                  </ScrollView>
              ...
              <SafeAreaView className="absolute bottom-0 left-0 right-0 bg-brand-black/95 border-t border-brand-green/30" style={{ height: layoutConfig.bottomPanelTotalHeight }} edges={["bottom"]}>
                <ScrollView className="flex-1 px-5 pt-3" showsVerticalScrollIndicator={false}>
                  ...
                  <SectionCard variant="secondary" className="mt-3"><Text className="text-white/80 text-xs leading-4 text-center">The green marker shows your original zero. The orange marker shows where you dialed. After dialing back, your scope should align with green.</Text></SectionCard>
                  {verifyPos && <SectionCard variant="elevated" className="mt-3"><Text className="text-white/50 text-xs text-center">Original Zero Position</Text><Text className="text-brand-greenLight font-mono text-xl font-bold text-center mt-1">({verifyPos.x}, {verifyPos.y})</Text></SectionCard>}
                  <View className="bg-brand-greenLight/10 border border-brand-greenLight/30 rounded-xl p-3 mt-3"><Text className="text-brand-greenLight/90 text-xs text-center font-medium">If aligned with GREEN, tap Continue. If not, go back and re-dial.</Text></View>
